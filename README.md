...
- [Pillar 1 — Fluid Execution](#pillar-1--fluid-execution-slicing-complete-rpc-bridge-in-phase-15)
- [Pillar 2 — Semantic UI](#pillar-2--semantic-ui-phase-3-complete)
- [Pillar 3 — Zero-Fetch Sync](#pillar-3--zero-fetch-sync-implemented-deployment-pending)
...
│   ├── server/        ← Hono HTTP runtime answering the compiler's RPC calls
...
## Pillar 1 — Fluid Execution *(slicing complete; RPC bridge in Phase 1.5)*
...
**`.blazefw/UserDashboard.client.js`** — contains `UserCard`, replaces `getUser` with an RPC stub. The target form of that stub is:

```ts
// Target wire contract — emitted in Phase 1.5, never written by hand
export async function getUser(id: string) {
  return __blazefw_rpc('/api/__blazefw/getUser', { id });
}
```

> **Honest status:** bundle slicing (classifier + transformer) is complete and
> tested, and `@blazefw/server` answers `POST /api/__blazefw/:fnName` today.
> The compiler currently emits a *placeholder* stub rather than the fetch call
> above — stub emission and registry auto-generation are Phase 1.5 (see
> `packages/server/README.md`, "Compiler integration gaps").
...
## Pillar 3 — Zero-Fetch Sync *(implemented; deployment pending)*
...
> **Honest status:** all four layers below are implemented and test-covered,
> but the sync server has not yet been deployed and operated under a real
> workload — that is the Phase 2 checkpoint of the server roadmap
...
│   ├── server/                @blazefw/server — HTTP RPC runtime (Hono, Dockerised)
...
### Full-stack deployment (with RPC + Zero-Fetch Sync)

The server half — `@blazefw/server` (HTTP RPC runtime) now, `@blazefw/sync-server` (CRDT WebSockets) in Phase 3 — runs as a single long-running container, per [ADR-001](docs/adr/ADR-001-blazefw-runtime-model.md). A production multi-stage `Dockerfile` ships at the repo root (`node:22-slim`, non-root, `HEALTHCHECK` on `/health`):

```bash
docker build -t blazefw-server .
docker run -p 3000:3000 blazefw-server
```

Target platforms:

- **Fly.io** — the chosen deploy target (server roadmap Phase 2)
- **Railway**
- **Cloudflare Durable Objects** — future roadmap target for the sync server

### CI pipeline

A GitHub Actions workflow (`.github/workflows/deploy.yml`) runs on every push and PR to `main` with three jobs: the monorepo build without Rust (confirming the WASM fallback path), the `create-blazefw` scaffold E2E with the real compiler, and the server job — `@blazefw/server` unit tests plus a Docker image build smoke-tested against `/health` and the RPC route.
...
### Phase 2 — Fluid Execution ✅ *(slicing — RPC fetch-stub emission is Phase 1.5)*

| Task | Description |
|---|---|
| 2.1 | `CapabilityScanner` — detects `window`, `document`, `localStorage` |
| 2.2 | `SecretScanner` — detects `process.env`, DB imports, `node:` protocol |
| 2.3 | `Slicer` — Classifier (5 DeclKinds) + Transformer + placeholder RPC stub injection |
...
### Phase 4 — Zero-Fetch Sync ✅ *(implemented; production deployment pending)*
...
### Server Runtime — Week 1 ✅ *(server roadmap, [ADR-001](docs/adr/ADR-001-blazefw-runtime-model.md))*

| Gate | Description |
|---|---|
| 0 | ADR-001 accepted (long-running container, Fly.io target); RPC wire contract documented |
| 1 | `@blazefw/server` — Hono + `/health` (status, uptime, version) |
| 2 | Middleware spine — request-id, structured JSON logging, no-leak error handler |
| 3 | `POST /api/__blazefw/:fnName` RPC route + function registry (15 tests) |
| 4 | Web → server round trip via Vite dev proxy |
| 5 | Production Dockerfile — multi-stage, `node:22-slim`, non-root, HEALTHCHECK |

Next on the server roadmap: Fly.io deploy + operations (Phase 2), compiler
RPC-stub emission + registry manifest (Phase 1.5), CRDT sync persistence (Phase 3).
